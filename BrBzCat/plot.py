...
data11 = np.genfromtxt('brbzcat_unrel.txt')
#ip11 = 2911.85
stcksx11 = data11[:,0] 
stcksy11 = (data11[:,1])
data11 = np.genfromtxt('brbzcat_unrel.dat')
x11 = data11[:,0] 
y11 = (data11[:,1])

#
data22 = np.genfromtxt('brbzcat_rel.txt')
#ip22 = 291.90
stcksx22 = data22[:,0] 
stcksy22 = (data22[:,1])
data22 = np.genfromtxt('brbzcat_rel.dat')
x22 = data22[:,0] 
y22 = (data22[:,1])

#Experiment
data10 = np.genfromtxt('BrBzexp.csv')
ip10 = 0.0 #aprox
x10 = data10[:,1] 
y10 = (data10[:,0])
...
plt.plot(x1+x1shift, y1,'crimson', label='BrBz', linewidth=2)
plt.plot(x11+x1shift, y11, 'darkblue', label='BrBz$^+$ (unrelaxed)', linewidth=2)#, linestyle='--')
plt.plot(x22+x1shift, y22, 'green', label='BrBz$^+$ (relaxed)', linewidth=2)#, linestyle='--')
#plt.plot(x2+x1shift, y2*1, 'darkblue', label='S1($n\pi^*$)  x 10', linewidth=2)
#plt.plot(x2+x1shift, y2*1, 'darkblue', label='1\B1u', linewidth=2)
#plt.plot(x3+x1shift, y3*1, 'g', label='S2 ($\pi\pi^*$)  x 10', linewidth=2)
#plt.plot(x3+x1shift, y3*2, 'g', label='2\B2u + 2\B3u', linewidth=2)
...
n=len(stcksx11)
for i in range(n):
    plt.plot([stcksx11[i]+x1shift,stcksx11[i]+x1shift],[0,stcksy11[i]*1],'darkblue',linewidth=1.0)
n=len(stcksx22)
for i in range(n):
    plt.plot([stcksx22[i]+x1shift,stcksx22[i]+x1shift],[0,stcksy22[i]*1],'green',linewidth=1.0)
#n=len(stcksx2)
#for i in range(n):
#    plt.plot([stcksx2[i]+x1shift,stcksx2[i]+x1shift],[0,stcksy2[i]*1],'darkblue',linewidth=1.0)
#n=len(stcksx3)
#for i in range(n):
#    plt.plot([stcksx3[i]+x1shift,stcksx3[i]+x1shift],[0,stcksy3[i]*2],'green',linewidth=1.0)
#n=len(stcksx4)
#for i in range(n):
...
plt.savefig('BrBzCat_NEXAFS.pdf')
plt.show()
